...
// StepResult helper functions for cleaner workflow definitions

// Next proceeds to the next step with the given message
func Next(message string) StepResult {
	return StepResult{NextOffset: 1, Message: message}
}

// Retry retries the current step with the given message
func Retry(message string) StepResult {
	return StepResult{NextOffset: 0, Message: message}
}

// GoTo jumps to a specific step by name
func GoTo(stepName, message string) StepResult {
	return StepResult{NextStep: stepName, Message: message}
}

// Back goes back one step with the given message
// Deprecated: Use GoTo with explicit step name for clarity
func Back(message string) StepResult {
	return StepResult{NextOffset: -1, Message: message}
}

// BackN goes back n steps with the given message
// Deprecated: Use GoTo with explicit step name for clarity
func BackN(n int, message string) StepResult {
	if n < 1 {
		n = 1
	}
...
// Skip skips forward by offset steps with the given message
// Skip(2, msg) skips 1 step (goes to current+2)
// Deprecated: Use GoTo with explicit step name for clarity
func Skip(offset int, message string) StepResult {
	if offset < 1 {
		offset = 1
	}
	return StepResult{NextOffset: offset, Message: message}
}
...
func WithSendPrompt(fn func(ctx context.Context, prompt string) <-chan string) WorkflowOption {
	return func(w *Workflow) {
		w.sendPrompt = fn
	}
}
...
func WithOnProgress(fn func(stepIndex int, stepState StepState, message string)) WorkflowOption {
	return func(w *Workflow) {
		w.onProgress = fn
	}
}
...
func WithRequestYesNo(fn func(question string) <-chan bool) WorkflowOption {
	return func(w *Workflow) {
		w.requestYesNo = fn
	}
}
...
func NewWorkflow(name string, db *storage.DB, repoInfo RepoInfo, opts ...WorkflowOption) *Workflow {
	host, org, project := parseProjectSlug(repoInfo.ProjectRoot)
	branch := branchSlugOrDefault(repoInfo.Branch)

	w := &Workflow{
		ID:          generateSessionID(), // Reuse session ID generation
		Name:        name,
		Steps:       []Step{},
		StepStates:  []StepState{},
		CurrentStep: 0,
...
func (w *Workflow) AddCmd(name, command, storeAs string) *Workflow {
	return w.Add(Step{
		...
		Verify: func(w *Workflow, response string) StepResult {
			...
			if err != nil {
				return Retry(fmt.Sprintf("❌ Command failed: %v", err))
			}
			if storeAs != "" {
				w.Set(storeAs, output)
			}
			return Next("✓ Command succeeded")
		},
	})
}
...
func (w *Workflow) AddGate(name string, condition func(w *Workflow) bool, waitMessage string) *Workflow {
	return w.Add(Step{
		...
		Verify: func(w *Workflow, response string) StepResult {
			if condition(w) {
				return Next("✓ Condition met")
			}
			return Retry(waitMessage)
		},
	})
}
...
func (w *Workflow) AddIf(condition func(w *Workflow) bool, step Step) *Workflow {
	...
	step.Verify = func(w *Workflow, response string) StepResult {
		if !condition(w) {
			return Next("⊘ Skipped (condition not met)")
		}
...
func (w *Workflow) AddRun(name string, fn func(w *Workflow) error) *Workflow {
	return w.Add(Step{
		...
		Verify: func(w *Workflow, response string) StepResult {
			if err := fn(w); err != nil {
				return Retry(fmt.Sprintf("❌ %v", err))
			}
			return Next("✓ Completed")
		},
	})
...
func LoadWorkflow(db *storage.DB, workflowID string) (*Workflow, error) {
...
// ListActiveWorkflows returns all active (pending/running) workflows for the current branch
func ListActiveWorkflows(db *storage.DB, repoInfo RepoInfo) ([]*Workflow, error) {
	store := storage.NewWorkflowStore(db)
	host, org, project := parseProjectSlug(repoInfo.ProjectRoot)
	branch := branchSlugOrDefault(repoInfo.Branch)

	workflowsData, err := store.ListActiveWorkflows(host, org, project, branch)
	if err != nil {
		return nil, err
	}
...
// runShellCommand executes a shell command and returns its combined output
